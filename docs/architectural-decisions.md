...
**Decisione**: la pipeline di analisi (discovery, scoring, citation check) vive in un microservizio Python su Hetzner separato da Vercel.
...
- ChatGPT e Perplexity non hanno API equivalenti. Perplexity ha un'API ma non espone le fonti con granularità sufficiente per il gap analysis.
...
**Implicazione**: se si aggiunge un nuovo use case che richiede embedding, usare Voyage AI. Non mescolare provider (confonde i confronti di cosine similarity).
...
- Il `country` e `search_lang` di Brave supportano la market-aware discovery (Task 5.9).
...
**Quando riconsiderare**: se si aggiungono job schedulati frequenti (citation checks automatici settimanali per migliaia di utenti), Redis + BullMQ o una soluzione equivalente sarà necessaria.
...
**Utilizzo**: `targetLanguage` viene passato a Gemini per il fan-out, a Brave come `search_lang`, e come parametro nei prompt del citation check. `targetCountry` viene passato a Brave come `country`.
