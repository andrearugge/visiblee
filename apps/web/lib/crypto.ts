 ...
 *
 * Cross-compatible with the Python `crypto_utils.py` in the analyzer service.
 */
...
function getKey(): Buffer {
  ...
  if (!hex || hex.length !== 64) {
    throw new Error('GSC_TOKEN_ENCRYPTION_KEY must be a 64-character hex string (32 bytes)');
  }
